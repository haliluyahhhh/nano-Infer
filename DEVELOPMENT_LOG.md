...
## 2025-03-18 采样增强：repetition_penalty + top-k + top-p

**类型**：feat  
**范围**：`engine/async_llm_engine.py`、`engine/sequence.py`、`config.py`、`entrypoints/offline_run.py`、`entrypoints/openai_api.py`、`docs/usage.md`

### 问题
- 模型生成存在大量重复（如「北京是中国的首都。北京是中国的首都…」），原采样仅支持 temperature / greedy。

### 修改
- `Sequence` 与 `EngineConfig` 新增 `top_p`、`top_k`、`repetition_penalty` 参数。
- `AsyncLLMEngine._sample`：对已出现 token 施加 repetition penalty（>1 抑制，与 HF 一致）→ temperature 缩放 → top-k 截断 → top-p 截断 → multinomial 采样。
- `offline_run.py`：CLI 默认 temperature=0.7、top_p=0.9、top_k=50、rep=1.2，可通过 `--rep` 等覆盖。
- `openai_api.py`：`ChatCompletionRequest` 同步增加三个字段，传入 `generate_tokens`。
